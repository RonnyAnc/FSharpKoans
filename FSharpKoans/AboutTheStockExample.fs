...
module ``about the stock example`` =
    ...
    let YouGotTheAnswerCorrect () =
        let toNumber = System.Double.Parse

        let selectRelevantData (x: string array) =
            (x[0], (toNumber x[1]), (toNumber x[4]))

        let differenceBetweenOpenAndClose (_, op, clo) = abs (op - clo)
        let takeDate (date, _, _) = date

        let result =
            stockData
            |> List.map (fun x -> x.Split([| ',' |]))
            |> List.skip 1
            |> List.map selectRelevantData
            |> List.maxBy differenceBetweenOpenAndClose
            |> takeDate
